...
function summary(entries: AuditEntry[]): string {
  const refusals = entries.filter((e) => e.result === 'refused').length;
  return `${entries.length} tool call${entries.length === 1 ? '' : 's'} · ${refusals} refusal${refusals === 1 ? '' : 's'}`;
}
  ...
  return (
    <Card radius="md">
      <CardHeader
        ...
        aside={<span className="mono text-[11px] text-muted">{summary(entries)}</span>}
      />
      {entries.length === 0 ? (
        <SectionEmpty>No agent activity has been recorded for this case yet.</SectionEmpty>
      ) : (
        <ul>
          {entries.map((entry, i) => (
            <li
              key={`${entry.time}-${entry.tool}-${i}`}
...
export function AuditTimeline({ entries }: { entries: AuditEntry[] }) {
  if (entries.length === 0) {
    return <div className="text-[12.5px] text-muted">No agent activity has been recorded yet.</div>;
  }
  return (
    <ol className="flex flex-col gap-3">
      {entries.map((entry, i) => (
        <li key={`${entry.time}-${entry.tool}-${i}`}>
